"""Soft-pause webhook receiver (recommended response loop).

First-class companion to Burnwatch soft pause: dashboard Pause, auto-pause on critical, and SDK should_pause().
...
class Handler(BaseHTTPRequestHandler):
    def do_POST(self) -> None:
        ...
        if alert.get("event") == "agent.paused" or (
            alert.get("event") == "detection.created" and alert.get("severity") in ACT_ON
        ):
            pause_agent(
                alert.get("agent_id", ""),
                alert.get("agent_name", "unknown"),
                alert.get("summary")
                or alert.get("paused_reason")
                or alert.get("type", "anomaly"),
            )
...
if __name__ == "__main__":
    ...
    print(f"Burnwatch soft-pause webhook listening on :{port} (detection.{sorted(ACT_ON)} + agent.paused)")
